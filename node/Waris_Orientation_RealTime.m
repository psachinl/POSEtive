...
%   ahrs = MadgwickAHRS('SamplePeriod', samplePeriod, 'Beta', 0.1);
    ...
    linVel(end,:) = linVel(end-1,:) + linAcc(end,:) * samplePeriod;
    ...
    linPos(end,:) = linPos(end-1,:) + linVelHP(end,:) * samplePeriod;
    ...
    ModSixDOFanimation(linPosHP, R, 'Figure', fig1);
    clf(fig1)
