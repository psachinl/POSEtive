...
input_file = 'Axis3T.txt'; 

csv_data = csvread(input_file,1);
data = csv_data;

[~,idx]=unique(data,'rows','first');
out=data(idx,:);

[acc,gyr,mag]=splitData(out);
clear csv_data
...
SamplePlotFreq = 1;
ModSixDOFanimation(linPosHP, R);
% SixDOFanimation(linPosHP, R, ...
%                 'SamplePlotFreq', SamplePlotFreq, 'Trail', 'Off', ...
%                 'LimitRatio',1, ...
%                 'Position', [9 39 1280 720], ...
%                 'AxisLength', 1, 'ShowArrowHead', 'on', ...
%                 'Xlabel', 'X (m)', 'Ylabel', 'Y (m)', 'Zlabel', 'Z (m)', 'ShowLegend', 'true', 'Title', 'Unfiltered',...
